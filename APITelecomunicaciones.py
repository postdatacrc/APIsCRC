...
def GetAllColumns(resourceid):
    ...
    response = urlopen(consulta, context=ctx) 
...
def API(resourceid,columns_fields,columns,filters,metric,grouping_col):
    ...
    consulta='https://www.postdata.gov.co/api/action/datastore/search.json?resource_id=' + resourceid + ''\
             ...
             +'&group_by='+columns\
             +'&'+metric+'='+grouping_col 
    response_base = urlopen(consulta.replace(" ", "%20")+ '&limit=10000000', context=ctx) 
...
def columns_dict(x):
    response = requests.get(url_dict[x], verify=False)
